...
def ldvi_trainer(
    ...
    params_flat, unflatten, params_fixed = initialize_ldvi(
    ...
    for i in range(iters):
        ...
        #     logger["gamma"] = collect_gamma(params_flat, unflatten, trainable)
        #     logger["stats/step"] = i
        ...
        if (i % eval_freq == 0) or (i == iters - 1):
            ...
            logger["other/gamma"] = [collect_gamma(params_flat, unflatten, trainable)]
            logger["stats/step"] = [i]
            logger["stats/wallclock"] = [timer]
            logger["stats/nfe"] = [(i + 1) * batch_size * 2 * (alg_cfg.num_temps - 1)]
            logger.update(evaluate(params_flat, key))
