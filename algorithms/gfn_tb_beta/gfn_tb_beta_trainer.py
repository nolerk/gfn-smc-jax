...
from algorithms.gfn_tb_beta.buffer import build_terminal_state_buffer
from algorithms.gfn_tb_beta.gfn_tb_beta_rnd import rnd, loss_fn
from algorithms.gfn_tb_beta.utils import get_invtemp
from eval.utils import extract_last_entry
from utils.logger import log
from utils.print_utils import print_results


def gfn_tb_beta_trainer(cfg, target):
    ...
    target.visualise = partial(target.visualise, constrained=True, map_samples=False)
    target.sample = partial(target.sample, constrained=True)
    target.log_prob = partial(target.log_prob, constrained=True)

    target_xs = target.sample(jax.random.PRNGKey(0), (cfg.eval_samples,))

    # Define initial and target density
    # fmt: off
    # initial_dist = distrax.Independent(
    #     distrax.Uniform(low=target.min_coord * jnp.ones(dim), high=target.max_coord * jnp.ones(dim)), 1
    # )
    initial_dist = None
    aux_tuple = (dim,)
    ...
    model_state = init_model(key, dim, alg_cfg, bounded=True)
    ...
    def loss_bwd_grad_fn(
        ...
        return loss_fn_base(
            key, model_state, params, rnd_p, invtemp=invtemp
        )
