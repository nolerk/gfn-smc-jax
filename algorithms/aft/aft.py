...
def get_initial_samples_log_weight_tuples(
    ...
    log_weights_tuple = LogWeightsTuple(
        *[-jnp.log(batch) * jnp.ones(batch) for batch in batch_sizes]
    )
...
def update_tuples(
    ...
    step: int,
    cfg,
) -> Tuple[SamplesTuple, LogWeightsTuple, AcceptanceTuple]:
...
def flow_estimate_step(
    ...
    train_vfe, flow_grads = free_energy_and_grad(
        loop_state.flow_params, train_samples, train_log_weights, outer_step
    )
    validation_vfe = free_energy_eval(
        loop_state.flow_params, validation_samples, validation_log_weights, outer_step
    )
    ...
    new_best_params = jax.lax.cond(
        validation_vfe_is_better,
        lambda _: loop_state.flow_params,
        lambda _: loop_state.best_params,
        operand=None,
    )
    new_best_validation_vfe = jnp.where(
        validation_vfe_is_better, validation_vfe, loop_state.best_validation_vfe
    )
    new_best_index = jnp.where(
        validation_vfe_is_better, loop_state.inner_step, loop_state.best_index
    )
    ...
    new_flow_params = optax.apply_updates(loop_state.flow_params, updates)
    new_inner_step = loop_state.inner_step + 1

    # Pack everything into the next loop state.
    new_state_tuple = OptimizationLoopState(
        new_opt_state,
        new_flow_params,
        new_inner_step,
        new_opt_vfes,
...
def flow_estimation_should_continue(
    loop_state: OptimizationLoopState, opt_iters: int, stopping_criterion: str
) -> bool:
...
def optimize_free_energy(
    ...
    final_loop_state = jax.lax.while_loop(cond_fun, body_fun, initial_loop_state)
...
def inner_loop(
    ...
    log_normalizer_increment = flow_transport.get_log_normalizer_increment(
        samples_tuple.test_samples,
        log_weights_tuple.test_log_weights,
        flow_apply,
        flow_params,
        log_density,
        step,
...
def outer_loop_aft(
    ...
    smc_nfe = 0.0  # 2 * cfg.algorithm.batch_size * cfg.algorithm.num_temps
    mcmc_nfe = 0.0  # cfg.algorithm.batch_size * cfg.algorithm.num_temps * cfg.mcmc_cfg.hmc_num_leapfrog_steps * \
    ...
    if cfg.compute_forward_metrics and (target_samples is not None):
        ...
        for step in range(num_temps - 1, 0, -1):
            subkey, key = jax.random.split(key)
            samples, log_weights, incs, _ = reverse_inner_loop_jit(
                subkey, flow_params_all[step - 1], samples, log_weights, step
            )
