...
class PISGRADNetBounded(nn.Module):
    dim: int
    ...
    def setup(self):
        ...
        if self.learn_flow:
            ...
            if not self.share_embeddings:
                ...
                self.flow_timestep_coeff = jnp.linspace(
                    start=0.1, stop=100, num=self.flow_num_hid
                )[None]
    ...
    def get_flow_fourier_features(self, timesteps):
        sin_embed_cond = jnp.sin(
            (self.flow_timestep_coeff * timesteps) + self.flow_timestep_phase
        )
        cos_embed_cond = jnp.cos(
            (self.flow_timestep_coeff * timesteps) + self.flow_timestep_phase
        )
    ...
    def __call__(self, input_array, time_array, lgv_term=None, is_fwd=True):
        ...
        if is_fwd:
            ...
            out_state = self.fwd_net(out_state)
        else:
            out_state = self.bwd_net(out_state)

        out_state = jnp.clip(out_state, 0, self.outer_clip)
        ...
        if self.learn_flow:
            ...
            if not self.share_embeddings:
                ...
                flow_extended_input = jnp.concatenate(
                    (input_array, flow_t_net1), axis=-1
                )
            else:
