...
def get_short_inner_loop(
    markov_kernel_by_step: MarkovKernelApply, density_by_step: LogDensityByStep, config
):
...
def get_short_reverse_inner_loop(
    markov_kernel_by_step: MarkovKernelApply, density_by_step: LogDensityByStep, config
):
    """Get a short version of inner loop."""
...
def outer_loop_smc(
    ...
    mcmc_nfe = 0.0  # alg_cfg.batch_size * (alg_cfg.num_temps - 1) * (
    ...
    if cfg.compute_forward_metrics and (target_samples is not None):
        ...
        for step in range(num_temps, 1, -1):
            subkey, key = jax.random.split(key)
            fwd_samples, log_weights, incs, _ = reverse_inner_loop_jit(
                subkey, tar_samples, log_weights, step
            )
