...
from algorithms.pcula.pcula_rnd import neg_elbo, rnd
...
def pcula_trainer(cfg, target):
    ...
    print(f"Target Z = {jnp.exp(target.log_Z)}")

    # We run langevin dynamics wrt to energy on domain
    ...
    target_samples = target.sample(
        jax.random.PRNGKey(0), (cfg.eval_samples,), constrained=True
    )
    ...
    projection_fn = target.projection_fn
    penalty_strength = alg_cfg.penalty_strength
    ...
    for step in range(alg_cfg.iters):
        ...
        if (step % eval_freq == 0) or (step == alg_cfg.iters - 1):
            ...
            if cfg.use_logger:
                log({"loss": jnp.mean(losses)}, step=step)
